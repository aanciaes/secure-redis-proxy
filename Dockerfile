...
COPY --from=builder /home/gradle/project/build/libs/secure-redis-proxy-0.4.1.jar .
COPY --from=builder /home/gradle/project/src/main/resources/spring-application.yml .

COPY ./keystores/ ./keystores

# Run jar
CMD java -jar -Dspring.config.location=spring-application.yml ./secure-redis-proxy-0.4.1.jar
